...
- **总览（一键打包）**：对 ①UE 打包 → ②Web 构建 → ③复制压缩（发布）→ ④上传 四阶段做一键顺序编排。每个阶段卡保留对应分页的**全部参数**（引擎目录/保存与恢复、BuildConfig、跳过 C++/清 Cook/关 Editor；dev/prod 与目标目录、版本预览；发布编号/压缩工具/仅复制、「运行 XCC.exe」；上传本地文件/网盘目录/授权状态），未勾选的阶段自动跳过；任一阶段失败即中止（已产出物保留，可「重跑本阶段」）。运行中**每个阶段卡本身就是进度条**：fill 填充随该阶段执行增长 + 流光动画，✔ 完成 / ▶ 运行中 / ○ 待执行 / ⧗ 跳过 / ✖ 失败(红) 五态，顶部细条=整体进度（已完成启用阶段/启用总数）。服务端 `overviewStart/overviewPoll`：编排 job 复用四个既有执行器（不重复实现），逐阶段聚合日志与进度，可整体取消（级联取消当前子任务）。百度 App 凭据与 OAuth 配置仍在「上传」设置区，总览仅引用授权状态。
...
- UE 打包的 C++ 编译：未勾选“跳过 C++ 编译”时，**同时重建 Game 与 Editor 两个目标**（`XCC` 与 `XCCEditor`，Win64 + 当前 BuildConfig，参数形态与 UAT 一致含 `-remoteini`）——Editor 目标产出的 `UnrealEditor-XCC.dll` 是 Cook 序列化蓝图 CDO 的依赖，不同步重建会导致增量 Cook 产出与运行时 exe 不匹配（打包版加载蓝图报 `Bad export index` 崩溃）。**编译发生时会自动给 UAT 追加 `-clean`**（防旧序列化缓存）。UAT 前还会做**编译产物新鲜度硬校验**：`Binaries\Win64\XCC.exe`（或模块 DLL）与 `UnrealEditor-XCC.dll` 必须不早于 `Source`/`Plugins` 最新源文件，否则任务立即失败并给出文件与时间（即使勾选了“跳过编译”也拦截陈旧产物，绝不静默出旧包）。
