...
- **发布**：按当前工程名自动计算下一个发布名 `{项目名}-{yyyyMMdd}`（当天首个无编号）或 `{项目名}-{yyyyMMdd}-N`（之后 -1、-2…），编号可手动改并实时校验冲突；第三阶段实时显示压缩预览名称；**已授权时压缩预览名称（A）会与第四阶段「网盘目录」中按文件名排序的最新 `{项目名}-{yyyyMMdd}(-N).zip`（B）比对：A ≤ B 则在 B 基础上 +1（避免与网盘重名），A > B 保持 A；未授权或网盘查询失败时按原规则**；一键「复制命名」或「复制并压缩」（robocopy 复制 + 压缩；压缩工具自动优先 7-Zip，未安装时回退 .NET ZipFile，界面显示当前所用工具）；现有发布列表 + 运行按钮。**项目名自动解析优先级：自定义名 → 最近一次用过的名字（设置历史）→ 工程目录名**；现有发布的识别（文件夹与 zip）只按当前项目名/目录名/历史名的 `{名字}-{yyyyMMdd}(-N)` 模式，**不再注入任何内置默认前缀**。现有发布每行可「删除」：确认后**同名文件夹与 .zip 一并删除**（存在哪个删哪个；发布/一键打包/上传任务运行中禁用删除）。
- **上传（百度网盘）**：直连百度开放平台 PCS 分片上传，**不依赖 bdpan CLI**。在插件设置中填写 App Key / Secret Key / App ID / Redirect URI / 应用根目录（Secret 与 Token 仅以 Windows DPAPI 加密保存在 `~/.dsh/dsh-xcc-pack-tools/settings.json`，界面不回显），完成 OAuth 授权后：自动匹配最新发布 zip（可改本地文件），指定目标目录（相对应用根 `/apps/<应用目录>/`），按 4MB 分片直传；**每成功一片即持久化 uploadid + 完成清单（`~/.dsh/dsh-xcc-pack-tools/baidu-upload-state/`），取消/刷新/重启后再次上传相同文件与路径自动续传**；进度 = 已完成分片字节 / 文件大小（精确百分比、速度、剩余、ETA）。**已授权时「网盘目录」下方实时显示网盘上按文件名（项目+日期+编号）排序的最新 zip 文件名（不按文件创建/修改时间），「总览」第四阶段卡同步显示**；网盘查询走 PCS list 接口（分页 + 30s 缓存），失败不影响上传与命名。
