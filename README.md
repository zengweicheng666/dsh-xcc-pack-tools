...
- **发布**：按当前工程名自动计算下一个发布名 `{项目名}-{yyyyMMdd}`（当天首个无编号）或 `{项目名}-{yyyyMMdd}-N`（之后 -1、-2…），编号可手动改并实时校验冲突；第三阶段实时显示压缩预览名称；**已授权时压缩预览名称（A）会与第四阶段「网盘目录」中按文件名排序的最新 `{项目名}-{yyyyMMdd}(-N).zip`（B）比对：A ≤ B 则在 B 基础上 +1（避免与网盘重名），A > B 保持 A；未授权或网盘查询失败时按原规则**；一键「复制命名」或「复制并压缩」（robocopy 复制 + 压缩；压缩工具自动优先 7-Zip，未安装时回退 .NET ZipFile，界面显示当前所用工具）；现有发布列表 + 运行按钮。
