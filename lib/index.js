...
async function baiduShare(token, fsId) {
  const url = new URL('https://pan.baidu.com/rest/2.0/xpan/share');
  url.searchParams.set('method', 'create');
  url.searchParams.set('access_token', token);
  const data = await apiJson(url, {
    method: 'POST',
    headers: { 'content-type': 'application/x-www-form-urlencoded' },
    body: formBody({ fsid_list: JSON.stringify([Number(fsId)]), period: 0, schannel: 0 }),
  });
  return data.link ?? data.shorturl ?? data.share_url ?? null;
}
  ...
  api.uploadStart = async (p) => {
    ...
    (async () => {
      try {
        ...
        const shareLink = created.fs_id ? await baiduShare(token, created.fs_id).catch((error) => {
          job.lines.push(`分享链接生成失败：${error.message}`);
          return null;
        }) : null;
        job.stage = 'done';
        job.progress = { percent: 100, sent: size, total: size, etaSec: 0, label: '上传完成' };
        job.result = { localPath, remotePath: created.path ?? remotePath, fsId: created.fs_id, size, shareLink };
        job.lines.push(`上传完成：${created.path ?? remotePath}`);
        if (shareLink) job.lines.push(`分享链接：${shareLink}`);
        finishJob(job, { exitCode: 0 });
      } catch (error) {
        finishJob(job, { error });
      }
