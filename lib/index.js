...
import { decodeLine, localDateStamp, scanReleases, computeReleaseName, parseReleaseName, normalizeReleasePrefix, RELEASE_PREFIX, parseWebVersion, bumpWebVersion, versionText, resolveRemotePath, parseUproject, isVersionAssociation, ueVersionKey, parseLauncherInstalled, latestRemoteZipName, applyRemoteFloor } from './pure.js';
...
/** Read the configured release name for a project, or fall back to its folder name. */
async function effectiveReleaseProjectName(projectRoot) {
  ...
  if (typeof raw === 'string' && raw.trim() !== '') {
    try {
      const value = validateReleaseProjectName(raw);
      if (value !== '') return value;
    } catch { /* invalid hand-edited setting — use the safe automatic name */ }
  }
  return releaseProjectName(projectRoot);
}
...
function releasePrefixesForProject(projectRoot, projectName = releaseProjectName(projectRoot), historicalNames = []) {
  // Keep releases made before a custom name was configured visible. The
  // automatic directory basename is also a historical prefix after a user
  // changes the override, while RELEASE_PREFIX covers the original default.
  const candidates = [projectName, releaseProjectName(projectRoot), ...historicalNames, RELEASE_PREFIX];
  const seen = new Set();
  const prefixes = [];
  for (const candidate of candidates) {
    const prefix = normalizeReleasePrefix(candidate);
...
function buildApi(ctx) {
  ...
  api.root = async (p) => {
    ...
    const projectNameCustomized = projectName !== automaticProjectName;
  ...
  api.releaseProjectNameGet = async (p) => {
    ...
    const projectName = customName || automaticName;
    return {
      projectRoot,
      projectName,
      customName,
