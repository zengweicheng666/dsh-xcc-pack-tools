...
/**
 * Sample a process's cumulative IO READ bytes via the CIM per-process raw
 * counter (Win32_PerfRawData_PerfProc_Process.IOReadBytesPersec). Used to
 * derive upload progress/speed/ETA since bdpan prints no progress in
 * non-interactive mode.
 * WHY READ and not WRITE: Windows process IO counters only cover disk/file
 * I/O — network socket writes are NOT counted (verified: IOWrite stayed at
 * 0 while uploading). bdpan upload = read local zip (counted) + write
 * socket (not counted), so read bytes ≈ uploaded bytes (verified: reading a
 * 2MB file yields IORead delta = 2110976).
...
function readProcessIoBytes(pid) {
  return new Promise((resolve) => {
    const child = spawn('powershell.exe', ['-NoProfile', '-Command',
      `(Get-CimInstance Win32_PerfRawData_PerfProc_Process -Filter "IDProcess=${Number(pid)}" -ErrorAction SilentlyContinue).IOReadBytesPersec`],
    { windowsHide: true });
    let out = '';
...
function spawnChild(job, cmd, args, opts = {}) {
  return new Promise((resolve, reject) => {
    ...
    job.childPid = child.pid; // used by the upload progress sampler
...
function createJob(kind) {
  ...
  const job = {
    ...
    killed: false,
    result: undefined,
    progress: undefined, // upload progress: { percent, speed, sent, total, etaSec }
    fileSize: 0,
    childPid: undefined,
    children: new Set(),
    stdin: undefined,
...
function buildApi(ctx) {
  ...
  api.uploadStart = async (p) => {
    ...
    (async () => {
      // Real-time progress via the process IO counter: bdpan itself prints no
      // progress in non-interactive mode, so we sample the child's
      // IO.WriteBytesCount (socket writes) every 2s to derive
      // speed / sent / percent / ETA.
      let sampler = null;
      let lastBytes = null;
      let lastAt = Date.now();
      let baseline = 0;
      try {
        sampler = setInterval(async () => {
          ...
          const bytes = await readProcessIoBytes(pid);
          ...
          if (lastBytes !== null && bytes >= lastBytes) {
            ...
            if (dt > 0.5) {
              ...
              const sent = Math.min(Math.max(bytes - baseline, 0), fileSize || Number.MAX_SAFE_INTEGER);
              const percent = fileSize > 0 ? Math.min(99, Math.round((sent / fileSize) * 100)) : undefined;
              const etaSec = speed > 0 && fileSize > 0 && sent < fileSize ? (fileSize - sent) / speed : undefined;
              job.progress = { percent, speed, sent, total: fileSize, etaSec };
            }
        ...
        if (fileSize > 0) {
          job.progress = { percent: 100, speed: undefined, sent: fileSize, total: fileSize, etaSec: 0 };
        }
        job.stage = 'done';
        job.result = { localPath, remote, name: zipName, size: fileSize };
        job.lines.push(`上传完成：/apps/${remote}${fileSize > 0 ? `（${(fileSize / 1073741824).toFixed(2)} GB）` : ''}`);
        finishJob(job, { exitCode: code });
      } catch (e) {
    ...
    return { jobId: job.jobId, kind: 'upload', localPath, remote, name: zipName, size: fileSize };
  };
