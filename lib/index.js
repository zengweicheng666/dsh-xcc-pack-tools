...
function buildApi(ctx) {
  ...
  api.uploadStart = async (p) => {
    ...
    // progress: bdpan pre-reads the whole file (checksum burst) before
    // uploading — rebaseOnBurst collapses that so percent tracks the upload.
    const stopSampler = startIoProgressSampler(job, { counter: 'read', total: fileSize, rebaseOnBurst: true });
    (async () => {
      try {
        const code = await spawnChild(job, bin, ['upload', localPath, remote], { cwd: projectRoot });
        if (code !== 0) throw new Error(`上传失败（退出码 ${code}）。`);
        if (fileSize > 0) {
          job.progress = { percent: 100, speed: undefined, sent: fileSize, total: fileSize, etaSec: 0, label: '上传完成' };
        }
        job.stage = 'done';
