...
/** Compile the project with UnrealBuildTool (mirrors package.ps1's UBT step). */
async function compileProject(job, projectRoot, ubtPath, uproject, projectName, buildConfig) {
  const args = [
    projectName, 'Win64', buildConfig,
    `-Project=${uproject}`,
    '-WaitMutex',
    '-FromMsBuild',
  ];
  job.lines.push(`UBT: ${ubtPath} ${args.join(' ')}`);
  const code = await spawnChild(job, ubtPath, args, { cwd: projectRoot });
  if (code !== 0) throw new Error(`UE C++ 编译失败（退出码 ${code}）。`);
  job.lines.push('C++ 编译完成');
}
 ...
 */
async function runUat(job, projectRoot, engineDir, uproject, projectName, buildConfig, nocompile, clean) {
  ...
  api.packStart = async (p) => {
    ...
    (async () => {
      try {
        ...
        plan.push({ name: 'Cook & 打包（UAT）', fn: () => runUat(job, projectRoot, engine.dir, uproject, projectName, buildConfig, skipCompile || !ubtExists, cleanCook) });
        const total = plan.length;
        for (let i = 0; i < total; i++) {
          const step = plan[i];
          const last = i === total - 1;
          // UAT (always last) shows an indeterminate bar; quick steps advance 0/…%
          job.progress = last ? { label: step.name } : { percent: Math.round((i / total) * 100), label: step.name };
