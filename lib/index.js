...
async function verifyBuildFreshness(job, projectRoot, projectName, buildConfig, validationMode = 'balanced') {
  ...
  if (problems.length > 0) {
    const diagnostic = {
      ...
      title: '编译产物可能未包含最新代码',
      reason: `检测到源码更新时间晚于现有二进制：${problems.join('；')}`,
      risk: '继续打包可能使用旧代码，或导致 Cook 序列化与运行时二进制不一致。',
      details: { newestSourceMtime: newest, newestSource: fmt(newest), binaries: [{ path: exe, mtime: exeT ? fmt(exeT) : '缺失' }, { path: modDll, mtime: modT ? fmt(modT) : '缺失' }, { path: edDll, mtime: edT ? fmt(edT) : '缺失' }] },
