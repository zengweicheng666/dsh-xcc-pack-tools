 ...
 * then Windows PATH lookup. Cached per process.
 */
let bdpanCache; // undefined = not probed, null = not found, string = path
async function detectBdpan() {
  if (bdpanCache !== undefined) return bdpanCache;
  const candidates = [
  ...
  return new Promise((resolve) => {
    execFile(bin, args, {
      ...
      resolve({
        ...
        stdout: decodeLine(stdout || Buffer.alloc(0)),
        stderr: decodeLine(stderr || Buffer.alloc(0)),
      });
    });
  });
}
...
function buildApi(ctx) {
  ...
  api.bdpanInstallStart = async () => {
    ...
    (async () => {
      try {
        ...
        if (code !== 0) throw new Error(`bdpan CLI 安装失败（退出码 ${code}）。`);
        job.stage = 'done';
        job.result = { binPath: await detectBdpan() };
  ...
  api.uploadStart = async (p) => {
    ...
    job.stage = 'uploading';
    job.lines.push(`bdpan upload ${localPath} ${remote}`);
    (async () => {
      try {
