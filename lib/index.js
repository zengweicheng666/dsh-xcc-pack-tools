 ...
 *   - Release      : copy Saved\Windows → Saved\<project>-{yyyyMMdd}(-N)\
 *                    then zip it as Saved\<project>-{yyyyMMdd}(-N).zip
...
import { decodeLine, localDateStamp, scanReleases, computeReleaseName, parseReleaseName, normalizeReleasePrefix, resolveEffectiveReleaseProjectName, releasePrefixCandidates, parseWebVersion, bumpWebVersion, versionText, resolveRemotePath, parseUproject, isVersionAssociation, ueVersionKey, parseLauncherInstalled, latestRemoteZipName, applyRemoteFloor } from './pure.js';
...
const INTERNAL_RELEASE_PROJECT_NAME = Symbol('internal release project name');
...
function writeError(res, error) {
  ...
  const diagnostic = error?.diagnostic && typeof error.diagnostic === 'object' ? error.diagnostic : undefined;
...
function diagnosticError(message, diagnostic, status = 409) {
  return Object.assign(new Error(message), {
    code: diagnostic?.code || 'diagnostic',
    status,
    diagnostic: {
      code: diagnostic?.code || 'diagnostic',
      severity: diagnostic?.severity || 'blocker',
      title: diagnostic?.title || message,
      risk: diagnostic?.risk,
      reason: diagnostic?.reason,
      details: diagnostic?.details,
      actions: diagnostic?.actions,
      canContinue: diagnostic?.canContinue === true,
    },
  });
}
...
// UAT output can contain a small bootstrap executable at the archive root and
// the real game executable below <Project>\\Binaries\\Win64.  Never assume a
// fixed filename: discover candidates from the actual output directory and
// rank the project executable/bootstrap ahead of UE/runtime helper programs.
const PACKAGED_HELPER_EXE_NAMES = new Set([
...
function isPackagedHelperExe(name) {
  const lower = String(name || '').toLowerCase();
  return PACKAGED_HELPER_EXE_NAMES.has(lower)
    || /^(?:unrealeditor(?:-cmd)?|unrealpak|unreallightmass|automationtool)(?:[-_.]|$)/.test(lower)
    || /^crashreportclient(?:[-_.]|$)/.test(lower);
}

async function scanPackagedExecutables(rootDir, maxDepth = 8) {
  const found = [];
  const queue = [{ dir: rootDir, depth: 0 }];
  const visited = new Set();
  while (queue.length > 0) {
    const current = queue.shift();
    const key = path.resolve(current.dir).toLowerCase();
    ...
    for (const entry of entries) {
      // Do not follow links out of the packaged tree.
      ...
      if (isPackagedHelperExe(entry.name)) continue;
      try {
...
function packagedExecutableRank(candidate, gameName) {
  const name = candidate.name.toLowerCase();
  const game = String(gameName || '').trim().toLowerCase();
  const exact = game !== '' && name === `${game}.exe`;
  const gameVariant = game !== '' && (name.startsWith(`${game}-win64-`) || name.startsWith(`${game}-`));
  if (exact && candidate.depth === 0) return 0; // conventional UAT bootstrap
  if (gameVariant && candidate.depth === 0) return 1;
  // A nested project binary is still a better launch target than an
  // unrelated root-level executable (for example a packaging helper).
  if (exact) return 2;
  if (gameVariant) return 3;
  const relative = String(candidate.relativePath || '').replace(/\\/g, '/').toLowerCase();
  if (/(^|\/)binaries\/win64\//.test(relative)) return 4;
  if (candidate.depth === 0) return 5;
  return 10 + candidate.depth;
}

async function findPackagedExecutable(rootDir, gameName) {
  const executables = await scanPackagedExecutables(rootDir);
  executables.sort((a, b) => {
    const rank = packagedExecutableRank(a, gameName) - packagedExecutableRank(b, gameName);
  ...
  return {
    executable: executables[0]?.path ?? null,
    relativePath: executables[0]?.relativePath ?? null,
    executables,
  };
}

function releaseProjectName(projectRoot) {
  const base = path.basename(path.resolve(projectRoot)).trim();
  return base || 'XCC-Deluxe';
}

const RELEASE_PROJECT_NAME_INVALID_RE = /[<>:"/\\|?*\u0000-\u001f]/;
const WINDOWS_RESERVED_BASENAME_RE = /^(?:con|prn|aux|nul|com[1-9]|lpt[1-9])(?:\..*)?$/i;
const MAX_RELEASE_PROJECT_NAME_HISTORY = 64;

/** Validate one user-supplied release project name as a safe Windows path segment. */
function validateReleaseProjectName(raw) {
  if (typeof raw !== 'string') throw badRequest('发布项目名必须是字符串');
  const value = raw.trim();
  if (value === '') return '';
  if (value === '.' || value === '..') throw badRequest('发布项目名不能是 . 或 ..');
  if (/^-+$/.test(value)) throw badRequest('发布项目名不能只包含连字符');
  if (value.length > 255) throw badRequest('发布项目名不能超过 255 个字符');
  if (RELEASE_PROJECT_NAME_INVALID_RE.test(value)) {
    throw badRequest('发布项目名包含非法 Windows 文件名字符（不能包含 < > : " / \\ | ? *）');
  }
  if (/[ .]$/.test(value)) throw badRequest('发布项目名不能以空格或句点结尾');
  if (WINDOWS_RESERVED_BASENAME_RE.test(value)) throw badRequest('发布项目名不能使用 Windows 保留设备名（如 CON、NUL、COM1）');
  return value;
}

/** Effective release project name + how it was resolved (custom/history/directory). */
async function effectiveReleaseProjectNameInfo(projectRoot) {
  const settings = await readSettings();
  const key = engineKeyFor(projectRoot);
  let override = '';
  const raw = settings.releaseProjectNames?.[key];
  if (typeof raw === 'string' && raw.trim() !== '') {
    try {
      override = validateReleaseProjectName(raw);
    } catch { /* invalid hand-edited setting — ignore the override */ }
  }
  // Priority (user-confirmed): explicit custom name → the most recent name
  // the project actually used (history tail) → the project directory name.
  return resolveEffectiveReleaseProjectName({
    override,
    history: normalizedReleaseProjectNameHistory(settings.releaseProjectNameHistory?.[key]),
    directoryName: releaseProjectName(projectRoot),
  });
}

/** Read the effective release project name for a project. */
async function effectiveReleaseProjectName(projectRoot) {
  return (await effectiveReleaseProjectNameInfo(projectRoot)).name;
}

function projectPathOf(projectRoot, requestedPath) {
  const resolved = path.resolve(projectRoot, String(requestedPath ?? ''));
  const relative = path.relative(path.resolve(projectRoot), resolved);
  if (relative === '..' || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)) return null;
  return resolved;
}

function releasePrefixesForProject(projectRoot, projectName = releaseProjectName(projectRoot), historicalNames = []) {
  // Recognition is driven ONLY by the project's own names: the active
  // (possibly custom) release name, the project directory basename, and
  // persisted historical names — no built-in default prefix is injected.
  return releasePrefixCandidates({
    projectName,
    directoryName: releaseProjectName(projectRoot),
    historicalNames,
  });
...
/** Include persisted custom names so renaming a project never hides old releases. */
async function releasePrefixesForProjectAsync(projectRoot, projectName) {
  const settings = await readSettings();
  const key = engineKeyFor(projectRoot);
  const raw = settings.releaseProjectNameHistory?.[key];
  const history = normalizedReleaseProjectNameHistory(raw);
  return releasePrefixesForProject(projectRoot, projectName, history);
}

function normalizedReleaseProjectNameHistory(raw) {
  const out = [];
  const seen = new Set();
  ...
  for (const value of values) {
    ...
    try {
      const name = validateReleaseProjectName(value);
      if (name === '') continue;
      const key = name.toLowerCase();
      if (seen.has(key)) continue;
      seen.add(key);
      out.push(name);
    } catch { /* ignore invalid hand-edited history */ }
  }
  return out.slice(-MAX_RELEASE_PROJECT_NAME_HISTORY);
}

/** Prefer the active project-name prefix, then fall back to historical releases. */
function latestReleaseZip(releases, currentPrefix) {
  const normalizedCurrent = normalizeReleasePrefix(currentPrefix).toLowerCase();
  const zips = releases.filter((release) => release.isZip === true && !release.isDir);
  return zips.find((release) => (
    typeof release.releasePrefix === 'string'
    && normalizeReleasePrefix(release.releasePrefix).toLowerCase() === normalizedCurrent
  )) ?? zips[0] ?? null;
}
...
// Generated Unreal/IDE trees can contain files with source-like extensions
// (for example Intermediate\...\Module.X.cpp). They are build products, not
// user-edited source, and must not advance the freshness baseline.
const GENERATED_SOURCE_DIR_NAMES = new Set([
  '.git', '.vs', 'binaries', 'deriveddatacache', 'intermediate',
  'node_modules', 'obj', 'saved', 'temp', 'tmp', 'bin',
]);

function isGeneratedSourceDir(name) {
  return GENERATED_SOURCE_DIR_NAMES.has(String(name || '').toLowerCase());
}

/** Newest real .cpp/.h/.inl/.cs mtime under Source/ and plugin source trees. */
async function newestSourceMtime(projectRoot) {
  ...
  while (stack.length > 0) {
    ...
    for (const e of entries) {
      ...
      if (e.isDirectory()) {
        if (!isGeneratedSourceDir(e.name)) stack.push(full);
      } else if (e.isFile() && /\.(cpp|h|inl|cs)$/i.test(e.name)) {
        try {
...
function freshnessConfigAliases(buildConfig) {
  const value = String(buildConfig || 'Development').trim();
  if (value.toLowerCase() === 'debug') return ['Debug', 'DebugGame'];
  if (value.toLowerCase() === 'debuggame') return ['DebugGame', 'Debug'];
  return [value];
}

/**
 * Return the game/editor binary names UE may emit for the selected target.
 * A Game target is commonly monolithic (`Project.exe`), while modular or
 * older layouts use `Project-Win64-<Config>.(exe|dll)`. Editor targets use
 * `UnrealEditor-Project.dll`; Debug and DebugGame are both seen in practice.
 */
function freshnessCandidateNames(projectName, buildConfig) {
  const base = String(projectName || '').trim();
  const game = new Set([`${base}.exe`]);
  const editor = new Set([`UnrealEditor-${base}.dll`]);
  for (const config of freshnessConfigAliases(buildConfig)) {
    for (const ext of ['.exe', '.dll']) {
      game.add(`${base}-Win64-${config}${ext}`);
      game.add(`${base}-${config}${ext}`);
    }
    editor.add(`UnrealEditor-${base}-Win64-${config}.dll`);
    editor.add(`UnrealEditor-${base}-${config}.dll`);
  }
  return { game: [...game], editor: [...editor] };
}

/** Scan Binaries\Win64 for actual outputs instead of fabricating one DLL path. */
async function findFreshnessBinaries(binDir, projectName, buildConfig) {
  const expected = freshnessCandidateNames(projectName, buildConfig);
  const gameNames = new Set(expected.game.map((name) => name.toLowerCase()));
  const editorNames = new Set(expected.editor.map((name) => name.toLowerCase()));
  let entries;
  try {
    entries = await fs.readdir(binDir, { withFileTypes: true });
  } catch {
  ...
  for (const entry of entries) {
    ...
    const kind = gameNames.has(lower) ? 'game' : editorNames.has(lower) ? 'editor' : null;
    if (kind === null) continue;
    const full = path.join(binDir, entry.name);
    try {
      const st = await fs.stat(full);
      if (!st.isFile()) continue;
      artifacts.push({ kind, name: entry.name, path: full, mtimeMs: st.mtimeMs });
    } catch { /* file disappeared while scanning */ }
  }
  return { expected, artifacts };
}
...
/**
 * Freshness diagnostic: the packaged binaries are compared with the newest
 * source file. Strict mode aborts on stale/missing output; balanced and
 * compatibility modes record a warning and continue so projects that package
 * successfully from the UE Editor are not rejected by timestamp-only checks.
 */
async function verifyBuildFreshness(job, projectRoot, projectName, buildConfig, validationMode = 'balanced') {
  const newestInfo = await newestSourceMtime(projectRoot);
  const newest = newestInfo.mtimeMs;
  if (newest <= 0) return; // no sources at all — nothing to guard
  const binDir = path.join(projectRoot, 'Binaries', 'Win64');
  const scanned = await findFreshnessBinaries(binDir, projectName, buildConfig);
  const gameArtifacts = scanned.artifacts.filter((a) => a.kind === 'game');
  const editorArtifacts = scanned.artifacts.filter((a) => a.kind === 'editor');
  const fmt = (ms) => (ms ? new Date(ms).toLocaleString('zh-CN', { hour12: false }) : '缺失');
  for (const artifact of scanned.artifacts) {
    job.lines.push(`编译产物检查（${artifact.kind === 'game' ? '游戏' : '编辑器'}）：${artifact.path} — ${fmt(artifact.mtimeMs)}`);
  }
  if (gameArtifacts.length === 0) job.lines.push(`编译产物检查（游戏）：未找到匹配输出（尝试：${scanned.expected.game.join('、')}）`);
  if (editorArtifacts.length === 0) job.lines.push(`编译产物检查（编辑器）：未找到匹配输出（尝试：${scanned.expected.editor.join('、')}）`);
  const gameFresh = gameArtifacts.some((artifact) => artifact.mtimeMs >= newest);
  const editorFresh = editorArtifacts.some((artifact) => artifact.mtimeMs >= newest);
  const problems = [];
  if (!gameFresh) problems.push(`游戏产物陈旧/缺失（候选：${scanned.expected.game.join('、')}）`);
  if (!editorFresh) problems.push(`编辑器模块陈旧/缺失（候选：${scanned.expected.editor.join('、')}；Cook 序列化依赖它）`);
  if (problems.length > 0) {
    const binaryDetails = scanned.artifacts.map((artifact) => ({
      kind: artifact.kind,
      path: artifact.path,
      mtime: fmt(artifact.mtimeMs),
    }));
    const diagnostic = {
      code: 'build-freshness',
      severity: validationMode === 'strict' ? 'blocker' : 'warning',
      title: '编译产物可能未包含最新代码',
      reason: `检测到源码更新时间晚于实际构建产物：${problems.join('；')}`,
      risk: '继续打包可能使用旧代码，或导致 Cook 序列化与运行时二进制不一致。',
      details: {
        newestSourceMtime: newest,
        newestSource: fmt(newest),
        newestSourcePath: newestInfo.path,
        binaries: binaryDetails,
        expected: scanned.expected,
      },
      actions: [{ id: 'rebuild', label: '重新编译', kind: 'primary' }, { id: 'clean-retry', label: '清理后重试', kind: 'secondary' }, { id: 'continue', label: '继续打包', kind: 'danger' }],
      canContinue: validationMode !== 'strict',
    };
    if (validationMode === 'strict') {
      throw diagnosticError(`编译产物陈旧/缺失（${problems.join('；')}）：最新源码修改于 ${fmt(newest)}，产物未包含最新代码。请重试编译或清理 Intermediate 后重新打包。`, diagnostic);
    }
    job.diagnostics.push(diagnostic);
    job.lines.push(`⚠ 风险提示：${diagnostic.title}。${diagnostic.risk}`);
    job.lines.push(`编译产物新鲜度检查发现风险，按 ${validationMode} 策略继续。`);
    return;
  }
 ...
 */
async function runUat(job, projectRoot, engineDir, uproject, projectName, buildConfig, nocompile, clean, autoClean, cookErrorPolicy = 'surface') {
  ...
  if (cookErrorPolicy === 'ignore') {
    uatArgs.push('-SkipCookingErrorSummary', '-IgnoreCookErrors');
    job.lines.push('Cook 错误策略：允许忽略（可能生成不完整包，请在日志中复核）。');
    job.mayBeIncomplete = true;
    job.diagnostics.push({
      code: 'cook-errors-ignored', severity: 'warning', title: 'Cook 错误将被忽略',
      reason: '已向 UAT 传入 -IgnoreCookErrors。',
      risk: '即使 UAT 返回成功，包内仍可能存在未 Cook 的资源或运行时缺失。',
      actions: [{ id: 'review-cook-log', label: '复核 Cook 日志', kind: 'primary' }, { id: 'rerun-surface', label: '以显示错误模式重试', kind: 'secondary' }],
      canContinue: true,
    });
  } else {
    job.lines.push('Cook 错误策略：显示并按 UAT 结果处理（未启用忽略 Cook 错误）。');
  }
  ...
  if (code !== 0) {
    throw diagnosticError(`Cook & 打包失败（UAT 退出码 ${code}）。请查看任务日志中的最后一条 Cook/UAT 错误。`, {
      code: 'uat-failed', severity: 'blocker', title: 'UE Cook & 打包失败',
      reason: `RunUAT.bat 返回退出码 ${code}。`,
      risk: '未生成可验证的完整打包产物，不能继续发布或上传。',
      details: { exitCode: code, runUat: runUatBat, arguments: uatArgs },
      actions: [{ id: 'inspect-log', label: '查看任务日志', kind: 'primary' }, { id: 'clean-retry', label: '清理 Cook 后重试', kind: 'secondary' }], canContinue: false,
    });
  }
}
...
// readBaiduConfig decrypts the DPAPI secret blob via a PowerShell spawn on
// every call — far too expensive for the hot paths (release-name previews on
// every keystroke). Cache the result briefly and invalidate it whenever a
// Baidu settings/token write lands.
let baiduConfigCache = { at: 0, value: null };
function invalidateBaiduConfigCache() { baiduConfigCache = { at: 0, value: null }; }
async function readBaiduConfigCached() {
  const now = Date.now();
  if (baiduConfigCache.value && now - baiduConfigCache.at < 30000) return baiduConfigCache.value;
  const config = await readBaiduConfig();
  baiduConfigCache = { at: now, value: config };
  return config;
}
...
/**
 * List one netdisk directory (PCS xpan/file method=list), following
 * pagination up to 50 pages of 1000. Returns [{ name, isdir }] — no sizes,
 * no timestamps: the caller ranks uploads by NAME only.
 */
async function baiduListDir(token, dirPath) {
  const out = [];
  const limit = 1000;
  let start = 0;
  for (let page = 0; page < 50; page++) {
    ...
    const data = await apiJson(url, { signal: AbortSignal.timeout(20000) });
    const list = Array.isArray(data.list) ? data.list : [];
    for (const item of list) {
      const name = String(item?.server_filename ?? item?.filename ?? '');
      if (name === '') continue;
      out.push({ name, isdir: Number(item?.isdir) === 1 });
    }
...
// The netdisk-latest query backs BOTH the 上传/总览 "网盘最新" display and the
// release-name floor, so its result is cached per resolved remote path for a
// short window (preview effects refire on every keystroke).
const netdiskLatestCache = new Map(); // remotePath -> { at, latest, error }
const NETDISK_LATEST_TTL_MS = 30000;

/**
 * Resolve the newest `<prefix><yyyyMMdd>(-N).zip` upload in the configured
 * netdisk directory (authorized only). Never throws: failures yield
 * { latest: null, error } so callers degrade to local naming rules.
 */
async function queryNetdiskLatest(projectRoot, projectName, remoteDir) {
  const config = await readBaiduConfigCached();
  const authorized = !!(config.publicConfig.clientId && config.publicConfig.appId && config.publicConfig.redirectUri
    && config.publicConfig.remoteRoot && config.publicConfig.hasClientSecret && config.secret.refreshToken);
  if (!authorized) return { latest: null, error: null };
  const dir = String(remoteDir ?? '').trim() || `${projectName}/`;
  let remotePath;
  try {
    remotePath = resolveBaiduRemote(config.publicConfig.remoteRoot, dir);
    if (/\.zip$/i.test(remotePath)) remotePath = remotePath.replace(/\/[^/]*$/, ''); // dir input, not a file
  } catch (e) {
    return { latest: null, error: e instanceof Error ? e.message : String(e) };
  }
  const cached = netdiskLatestCache.get(remotePath);
  if (cached && Date.now() - cached.at < NETDISK_LATEST_TTL_MS) return cached;
  let latest = null;
  let error = null;
  try {
    const token = await baiduAccessToken(config);
    const entries = await baiduListDir(token, remotePath);
    latest = latestRemoteZipName(entries, normalizeReleasePrefix(projectName));
  } catch (e) {
    error = e instanceof Error ? e.message : String(e);
  }
  const result = { at: Date.now(), latest, error };
  netdiskLatestCache.set(remotePath, result);
  return result;
}
...
function finishJob(job, { exitCode, error }) {
  ...
  if (error !== undefined) {
    ...
    if (error?.diagnostic) job.errorDetails = error.diagnostic;
  }
...
function jobPoll(jobId) {
  ...
  return {
    ...
    diagnostics: job.diagnostics?.slice(-20),
    killed: job.killed || undefined,
    result: job.result,
    policy: job.policy,
    mayBeIncomplete: job.mayBeIncomplete || undefined,
...
/**
 * Force-delete a path (file or directory tree). Recursive fs.rm fails on
 * Windows when the tree contains read-only files (UAT outputs), so fall
 * back to PowerShell's Remove-Item -Force, which clears those attributes.
 */
async function forceDeletePath(target) {
  try {
    await fs.rm(target, { recursive: true, force: true, maxRetries: 3, retryDelay: 500 });
  } catch {
    const psPath = `'${String(target).replace(/'/g, "''")}'`;
    await new Promise((resolve, reject) => {
      execFile(PS, ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command',
        `Remove-Item -LiteralPath ${psPath} -Recurse -Force -ErrorAction Stop`],
        { windowsHide: true, timeout: 10 * 60 * 1000 },
        (error, _stdout, stderr) => {
          if (error) reject(new Error(`删除失败（${String(stderr || error.message || error).trim()}）`));
          else resolve();
        });
    });
  }
...
function buildApi(ctx) {
  ...
  api.root = async (p) => {
    ...
    const automaticProjectName = releaseProjectName(projectRoot);
    const projectNameInfo = await effectiveReleaseProjectNameInfo(projectRoot);
    const projectName = projectNameInfo.name;
    const projectNameCustomized = projectNameInfo.source === 'custom';
    const gameName = uproject ? path.basename(uproject, path.extname(uproject)) : projectName;
    const releasePrefix = normalizeReleasePrefix(projectName);
    const buildInfo = await findPackagedExecutable(outputDir, gameName);
    const buildExecutable = buildInfo.executable;
    const hasBuild = !!buildExecutable;
    ...
    const releases = await scanReleases(path.join(projectRoot, 'Saved'), await releasePrefixesForProjectAsync(projectRoot, projectName));
    const releaseArtifacts = await Promise.all(releases.map(async (release) => {
      const info = release.isDir ? await findPackagedExecutable(release.path, gameName) : { executable: null, executables: [] };
      return {
        ...release,
        executable: info.executable,
        executables: info.executables,
        hasExecutable: !!info.executable,
      };
    }));
    ...
    try {
      ...
      if (v) webVersion = { current: versionText(v), next: versionText(bumpWebVersion(v)), mode: v.mode ?? null };
    } catch { /* no version.json */ }
    const sevenZip = await detectSevenZip();
    const bdpanBin = await detectBdpan();
    const settings = await readSettings();
    const latestZip = latestReleaseZip(releases, releasePrefix);
    return {
      projectRoot,
      uproject,
      ...
      projectNameSource: projectNameInfo.source,
      autoProjectName: automaticProjectName,
      releasePrefix,
      gameName,
      artifacts: {
        build: { dir: outputDir, executable: buildExecutable, executables: buildInfo.executables, exists: hasBuild, valid: hasBuild },
        releases: releaseArtifacts.map((r) => ({ name: r.name, dir: r.isDir ? r.path : undefined, zip: r.isZip ? r.path : undefined, executable: r.executable, executables: r.executables, hasExecutable: r.hasExecutable, mtime: r.mtime, size: r.size })),
      },
      ...
      settings: {
        bdpanRemoteDir: settings.bdpanRemoteDir ?? '',
        releaseProjectName: projectName,
        releaseProjectNameAutomatic: automaticProjectName,
      },
      now: localDateStamp(),
    };
  };
  ...
  api.nextName = async (p) => {
    const cwd = sessionCwdOf(ctx, p.sessionId, p.cwd);
    const projectRoot = await findProjectRoot(p.projectRoot || cwd);
    const projectName = await effectiveReleaseProjectName(projectRoot);
    const releasePrefix = normalizeReleasePrefix(projectName);
    const releases = await scanReleases(path.join(projectRoot, 'Saved'), await releasePrefixesForProjectAsync(projectRoot, projectName));
    const date = p.date && /^\d{8}$/.test(p.date) ? p.date : localDateStamp();
    const info = computeReleaseName(releases, date, p.number, releasePrefix);
    // Netdisk floor (authorized only, and only when the caller asked for a
    // directory — the UI always does; agent tools/tests omit remoteDir and
    // keep the pure local rule). Never throws: failures degrade to `info`.
    let finalInfo = info;
    let netdisk = null;
    if (p.remoteDir !== undefined) {
      const remote = await queryNetdiskLatest(projectRoot, projectName, p.remoteDir);
      finalInfo = applyRemoteFloor(info, remote.latest, releasePrefix, releases);
      netdisk = { latest: remote.latest ? remote.latest.name : null, adjusted: finalInfo.adjusted };
    }
    return {
      ...finalInfo,
      projectName,
      releasePrefix,
      zipName: `${finalInfo.name}.zip`,
      dir: path.join(projectRoot, 'Saved', finalInfo.name),
      zip: path.join(projectRoot, 'Saved', `${finalInfo.name}.zip`),
      netdisk,
    };
  };
  ...
  api.packStart = async (p) => {
    ...
    const validationMode = p.validationMode ?? 'balanced';
    if (!['balanced', 'strict', 'compatibility'].includes(validationMode)) throw badRequest('validationMode 必须是 balanced / strict / compatibility');
    const cookErrorPolicy = p.cookErrorPolicy ?? (validationMode === 'compatibility' ? 'ignore' : 'surface');
    if (!['surface', 'ignore'].includes(cookErrorPolicy)) throw badRequest('cookErrorPolicy 必须是 surface 或 ignore');
    ...
    if (!engine.dir) {
      ...
      const detail = isVersionAssociation(engineAssociation)
        ...
        : '源码引擎 GUID 关联无法自动定位。';
      throw diagnosticError(`未能定位 UE 引擎：${projectName} 的 ${assoc}，${detail}请在本页「引擎目录」填写引擎根目录（需包含 Engine\\Build\\BatchFiles\\RunUAT.bat）并保存后重试。`, {
        code: 'engine-not-found', severity: 'blocker', title: '未找到可用的 UE 引擎',
        reason: detail, risk: '没有 RunUAT 无法启动 UE 打包。',
        details: { project: projectName, engineAssociation, searched: ['registry', 'launcher', 'common-install-roots'] },
        actions: [{ id: 'set-engine', label: '填写并保存引擎目录', kind: 'primary' }], canContinue: false,
      });
    }
    const projectName = uproject ? path.basename(uproject, path.extname(uproject)) : 'XCC';

    // ---- editor gate: strict mode preserves the historical hard stop;
    // balanced/compatibility expose the lock risk and let UAT decide whether
    // the open editor actually blocks this project.
    const editorRunning = await editorProcessesRunning();
    if (editorRunning && !closeEditor && validationMode === 'strict') {
      throw diagnosticError('Unreal Editor 正在运行，严格模式禁止在编辑器开启时打包。', {
        code: 'editor-running', severity: 'blocker', title: 'Unreal Editor 正在运行',
        reason: '编辑器可能持有 Binaries、Intermediate 或 Cook 目录的文件锁。',
        risk: 'UAT 可能失败，或读取到未保存/不一致的工程状态。',
        actions: [{ id: 'close-editor', label: '关闭编辑器后重试', kind: 'primary' }, { id: 'allow-close', label: '允许流程自动关闭', kind: 'secondary' }], canContinue: false,
      });
    }

    const job = createJob('pack');
    job.policy = { validationMode, cookErrorPolicy };
    job.stage = 'packaging';
    job.progress = { percent: 0, label: '准备启动' };
    if (editorRunning && !closeEditor) {
      job.diagnostics.push({ code: 'editor-running', severity: 'warning', title: 'Unreal Editor 仍在运行', reason: '检测到 UnrealEditor 进程，但未请求自动关闭。', risk: '如果当前工程文件被锁定，UAT 可能失败；请根据日志决定是否关闭编辑器后重试。', actions: [{ id: 'close-editor', label: '关闭编辑器后重试', kind: 'primary' }, { id: 'continue', label: '继续打包', kind: 'danger' }], canContinue: true });
      job.lines.push('⚠ 风险提示：Unreal Editor 仍在运行；未自动关闭，将继续尝试 UAT。');
    }
    (async () => {
      try {
        ...
        const plan = [];
        if (editorRunning && closeEditor) plan.push({ name: '检查/关闭 Unreal Editor', fn: () => closeRunningEditors(job, projectRoot) });
        ...
        if (!skipCompile && !ubtExists) {
          const diagnostic = { code: 'ubt-missing', severity: validationMode === 'strict' ? 'blocker' : 'warning', title: '未找到 UnrealBuildTool', reason: ubtPath, risk: '将跳过 C++ 编译并使用现有二进制，源码变更可能不会进入安装包。', details: { ubtPath }, actions: [{ id: 'install-ubt', label: '检查引擎工具链', kind: 'primary' }, { id: 'continue', label: '继续并跳过编译', kind: 'danger' }], canContinue: validationMode !== 'strict' };
          if (validationMode === 'strict') throw diagnosticError('未找到 UnrealBuildTool，严格模式禁止跳过编译。', diagnostic);
          job.diagnostics.push(diagnostic);
          job.lines.push(`⚠ 风险提示：${diagnostic.title}，将自动使用 -nocompile。`);
        }
        plan.push({ name: 'Cook & 打包（UAT）', fn: () => runUat(job, projectRoot, engine.dir, uproject, projectName, buildConfig, skipCompile || !ubtExists, cleanCook || (!skipCompile && ubtExists), !skipCompile && ubtExists, cookErrorPolicy) });
        ...
        for (let i = 0; i < total; i++) {
          ...
          if (last) {
            ...
            await verifyBuildFreshness(job, projectRoot, projectName, buildConfig, validationMode);
          }
          await step.fn();
        }
        job.lineHooks = undefined;
        ...
        const outputDir = path.join(projectRoot, 'Saved', 'Windows');
        const artifact = await findPackagedExecutable(outputDir, projectName);
        if (artifact.executable) job.lines.push(`检测到可运行程序：${artifact.executable}`);
        else job.lines.push('未在打包目录中检测到可运行程序（后续发布/运行前会再次扫描）。');
        job.result = { outputDir, executable: artifact.executable, executables: artifact.executables, policy: job.policy, mayBeIncomplete: job.mayBeIncomplete };
  ...
  api.releaseStart = async (p) => {
    const cwd = sessionCwdOf(ctx, p.sessionId, p.cwd);
    const projectRoot = await findProjectRoot(p.projectRoot || cwd);
    const { uproject } = await readUprojectInfo(projectRoot);
    const projectName = typeof p[INTERNAL_RELEASE_PROJECT_NAME] === 'string'
      ? validateReleaseProjectName(p[INTERNAL_RELEASE_PROJECT_NAME])
      : await effectiveReleaseProjectName(projectRoot);
    const releasePrefix = normalizeReleasePrefix(projectName);
    const gameName = uproject ? path.basename(uproject, path.extname(uproject)) : projectName;
    const defaultSrc = path.join(projectRoot, 'Saved', 'Windows');
    const requestedSrc = p.sourceDir ? projectPathOf(projectRoot, p.sourceDir) : defaultSrc;
    if (requestedSrc === null) throw badRequest('sourceDir 必须位于当前 UE 工程目录内。');
    const src = requestedSrc;
    const srcInfo = await findPackagedExecutable(src, gameName);
    let srcValid = false;
    try { srcValid = (await fs.stat(src)).isDirectory() && !!srcInfo.executable; } catch { /* missing or incomplete output */ }
    if (!srcValid) {
      throw Object.assign(new Error(`打包产物不可用于发布：${src} 中未找到可执行文件。可先完成 UE 打包，或通过 sourceDir 指定工程目录内的其他有效产物目录。`), {
        code: 'artifact-not-found', status: 409,
        diagnostic: { code: 'artifact-not-found', severity: 'blocker', title: '没有可发布的 UE 产物', reason: `发布来源目录未发现 .exe：${src}`, risk: '复制压缩会生成不完整发布包。', details: { sourceDir: src, executables: srcInfo.executables }, actions: [{ id: 'pack', label: '先执行 UE 打包', kind: 'primary' }, { id: 'choose-source', label: '指定其他产物目录', kind: 'secondary' }], canContinue: false },
      });
    }
    const releases = await scanReleases(path.join(projectRoot, 'Saved'), await releasePrefixesForProjectAsync(projectRoot, projectName));
    const date = p.date && /^\d{8}$/.test(p.date) ? p.date : localDateStamp();
    const info = computeReleaseName(releases, date, p.number, releasePrefix);
    // Same netdisk floor as the preview (nextName): when the 网盘目录 already
    // holds an equal-or-newer name, continue from it (+1) so the produced
    // zip never collides with the netdisk's latest. Authorized + remoteDir
    // aware only; failures degrade to the local name.
    const remote = p.remoteDir !== undefined ? await queryNetdiskLatest(projectRoot, projectName, p.remoteDir) : { latest: null };
    const finalInfo = applyRemoteFloor(info, remote.latest, releasePrefix, releases);
    if (finalInfo.collisions.length > 0) {
      throw conflictError(`${finalInfo.collisions.join('；')}。请更换编号或删除旧产物。`);
    }
    ...
    const dst = path.join(projectRoot, 'Saved', finalInfo.name);
    const zipPath = `${dst}.zip`;
    const releaseExecutable = srcInfo.executable && srcInfo.relativePath
      ? path.join(dst, srcInfo.relativePath)
      : null;
    ...
    (async () => {
      try {
        ...
        if (!doZip) {
          ...
          job.result = { dir: dst, name: finalInfo.name, zip: undefined, sourceDir: src, sourceExecutable: srcInfo.executable, executable: releaseExecutable };
        ...
        if (zipTool === '7z') {
          ...
          // entries are <project>-{name}/..., same convention as before.
          // -mx=5: default balanced level — paks are already compressed, so
          // mx=9 buys little size for a lot of CPU time.
          const sevenZip = await detectSevenZip();
          job.lines.push(`7-Zip: ${sevenZip} a -tzip -mx=5 ${zipPath} ${finalInfo.name}`);
          const zc = await spawnChild(job, sevenZip, ['a', '-tzip', '-mx=5', '-y', zipPath, finalInfo.name], { cwd: path.join(projectRoot, 'Saved') });
          if (zc !== 0 && zc !== 1) throw new Error(`压缩失败（7-Zip 退出码 ${zc}）。发布目录已生成，可重试压缩。`);
        } else {
          // The fallback PowerShell command must quote apostrophes in a
          // custom project name, otherwise the generated path literal would
          // be malformed (or allow command injection).
          const psEscapePath = (value) => `'${String(value).replace(/'/g, "''")}'`;
          const zipCmd = `Add-Type -AssemblyName System.IO.Compression.FileSystem; ` +
            `[System.IO.Compression.ZipFile]::CreateFromDirectory(${psEscapePath(dst)},${psEscapePath(zipPath)},[System.IO.Compression.CompressionLevel]::Optimal,$true)`;
        ...
        job.result = { dir: dst, zip: zipPath, name: finalInfo.name, zipTool, sourceDir: src, sourceExecutable: srcInfo.executable, executable: releaseExecutable };
    ...
    return { jobId: job.jobId, kind: 'release', name: finalInfo.name, dir: dst, zip: doZip ? zipPath : undefined, zipTool, sourceDir: src, sourceExecutable: srcInfo.executable, executable: releaseExecutable };
  };
  ...
  api.releases = async (p) => {
    ...
    const projectRoot = await findProjectRoot(p.projectRoot || cwd);
    const projectName = await effectiveReleaseProjectName(projectRoot);
    return { releases: await scanReleases(path.join(projectRoot, 'Saved'), await releasePrefixesForProjectAsync(projectRoot, projectName)) };
  };

  /**
   * Delete one existing release: the Saved\<name> folder and the same-name
   * <name>.zip are removed together (whichever exists). `name` must match
   * the project's release pattern (<项目名>{yyyyMMdd}(-N)), which also
   * guarantees a single path segment inside Saved\ (no traversal).
   */
  api.releaseDelete = async (p) => {
    const cwd = sessionCwdOf(ctx, p.sessionId, p.cwd);
    const projectRoot = await findProjectRoot(p.projectRoot || cwd);
    const projectName = await effectiveReleaseProjectName(projectRoot);
    const releasePrefix = normalizeReleasePrefix(projectName);
    const name = String(p.name ?? '').trim();
    const prefixes = await releasePrefixesForProjectAsync(projectRoot, projectName);
    if (!prefixes.some((prefix) => parseReleaseName(name, prefix) !== null)) {
      throw badRequest(`发布名格式非法（应为 ${releasePrefix}{yyyyMMdd} 或 ${releasePrefix}{yyyyMMdd}-N）`);
    }
    // Deleting the destination mid-copy/mid-upload would corrupt a running
    // task — refuse while any of those job kinds is active.
    if (active.has('release') || active.has('overview') || active.has('upload')) {
      throw conflictError('发布/一键打包/上传任务运行中，请等待完成或取消后再删除。');
    }
    const dir = path.join(projectRoot, 'Saved', name);
    const zip = path.join(projectRoot, 'Saved', `${name}.zip`);
    const deleted = { dir: false, zip: false };
    for (const [target, isDir] of [[zip, false], [dir, true]]) {
      let exists = false;
      try {
      ...
      if (!exists) continue;
      await forceDeletePath(target);
      deleted[isDir ? 'dir' : 'zip'] = true;
    }
    if (!deleted.dir && !deleted.zip) {
      throw conflictError(`发布产物不存在：${name}（可能已被删除）`);
    }
    return { name, deleted, dir, zip };
  };

  /**
   * Launch the selected packaged executable detached (fire-and-forget, not a
   * monitored job). Both build and release targets are scanned at request
   * time, so renamed projects and nested UAT layouts work without a fixed
   * executable filename.
   */
  api.run = async (p) => {
    const cwd = sessionCwdOf(ctx, p.sessionId, p.cwd);
    ...
    const gameName = uproject ? path.basename(uproject, path.extname(uproject)) : projectName;
    const releasePrefix = normalizeReleasePrefix(projectName);
    const releasePrefixes = await releasePrefixesForProjectAsync(projectRoot, projectName);
    let exe;
    let dir;
    if (p.target === 'build') {
      dir = path.join(projectRoot, 'Saved', 'Windows');
      exe = (await findPackagedExecutable(dir, gameName)).executable;
    } else if (p.target === 'release') {
      const name = String(p.name ?? '');
      if (!releasePrefixes.some((prefix) => parseReleaseName(name, prefix) !== null)) {
        throw badRequest(`发布名格式非法（应为 ${releasePrefix}{yyyyMMdd} 或 ${releasePrefix}{yyyyMMdd}-N）`);
      }
    ...
    if (!exe) throw conflictError(`未找到可运行程序：${dir}（已扫描目录中的 .exe，但没有可用候选）`);
  ...
  api.releaseProjectNameGet = async (p) => {
    const cwd = sessionCwdOf(ctx, p.sessionId, p.cwd);
    const projectRoot = await findProjectRoot(p.projectRoot || cwd);
    const settings = await readSettings();
    const automaticName = releaseProjectName(projectRoot);
    const raw = settings.releaseProjectNames?.[engineKeyFor(projectRoot)];
    let customName = '';
    if (typeof raw === 'string' && raw.trim() !== '') {
      try {
        customName = validateReleaseProjectName(raw);
      } catch {
        // A manually edited settings file must not make the name read fail.
        customName = '';
      }
    }
    const info = await effectiveReleaseProjectNameInfo(projectRoot);
    return {
      projectRoot,
      projectName: info.name,
      customName,
      automaticName,
      projectNameCustomized: customName !== '',
      projectNameSource: info.source,
      releasePrefix: normalizeReleasePrefix(info.name),
    };
  };

  api.releaseProjectNameSet = async (p) => {
    const cwd = sessionCwdOf(ctx, p.sessionId, p.cwd);
    const projectRoot = await findProjectRoot(p.projectRoot || cwd);
    const customName = validateReleaseProjectName(p.projectName ?? p.name ?? '');
    let settings = await readSettings();
    if (!settings || typeof settings !== 'object' || Array.isArray(settings)) settings = {};
    const key = engineKeyFor(projectRoot);
    const previousName = typeof settings.releaseProjectNames?.[key] === 'string'
      ? settings.releaseProjectNames[key].trim()
      : '';
    const history = normalizedReleaseProjectNameHistory(settings.releaseProjectNameHistory?.[key]);
    if (previousName !== '' && previousName.toLowerCase() !== customName.toLowerCase()) {
      history.push(previousName);
    }
    const normalizedHistory = normalizedReleaseProjectNameHistory(history);
    if (!settings.releaseProjectNames || typeof settings.releaseProjectNames !== 'object' || Array.isArray(settings.releaseProjectNames)) {
      settings.releaseProjectNames = {};
    }
    if (customName === '') delete settings.releaseProjectNames[key];
    else settings.releaseProjectNames[key] = customName;
    if (Object.keys(settings.releaseProjectNames).length === 0) delete settings.releaseProjectNames;
    if (!settings.releaseProjectNameHistory || typeof settings.releaseProjectNameHistory !== 'object' || Array.isArray(settings.releaseProjectNameHistory)) {
      settings.releaseProjectNameHistory = {};
    }
    if (normalizedHistory.length > 0) settings.releaseProjectNameHistory[key] = normalizedHistory;
    else delete settings.releaseProjectNameHistory[key];
    if (Object.keys(settings.releaseProjectNameHistory).length === 0) delete settings.releaseProjectNameHistory;
    await writeSettings(settings);
    const projectName = customName || releaseProjectName(projectRoot);
    return {
      projectRoot,
      projectName,
      customName,
      automaticName: releaseProjectName(projectRoot),
      projectNameCustomized: customName !== '',
      releasePrefix: normalizeReleasePrefix(projectName),
    };
  };
  ...
  /**
   * Newest `<project>-<yyyyMMdd>(-N).zip` already uploaded into the 网盘目录
   * (authorized only). Ranked by file NAME (date desc, number desc) — never by
   * upload/modify time. Consumed by the 上传 tab and the 总览 upload card, and
   * by nextName/releaseStart as the netdisk floor for the release name.
   */
  api.baiduDirLatest = async (p) => {
    const cwd = sessionCwdOf(ctx, p.sessionId, p.cwd);
    const projectRoot = await findProjectRoot(p.projectRoot || cwd);
    const projectName = await effectiveReleaseProjectName(projectRoot);
    const config = await readBaiduConfigCached();
    const authorized = !!(config.publicConfig.clientId && config.publicConfig.appId && config.publicConfig.redirectUri
      && config.publicConfig.remoteRoot && config.publicConfig.hasClientSecret && config.secret.refreshToken);
    if (!authorized) return { authorized: false, name: null, error: null };
    const result = await queryNetdiskLatest(projectRoot, projectName, p.remoteDir);
    return { authorized: true, name: result.latest ? result.latest.name : null, error: result.error };
  };
  ...
  api.uploadStart = async (p) => {
    ...
    // An empty network dir (client cleared the 网盘目录 input) must fall back to
    // the per-project default instead of reaching resolveRemotePath as "" — that
    // would throw「请填写网盘目标路径」and fail the upload job.
    const remoteDir = String(p.remoteDir ?? '').trim() || `${projectName}/`;
    const relative = resolveRemotePath(remoteDir, zipName);
    ...
    // authoritative destination of THIS upload (auto-matched zip name included);
    // surfaced to the 总览 upload card under the 网盘目录 row via jobPoll
  ...
  api.overviewStart = async (p) => {
    ...
    // The release-name netdisk floor compares against the ④ 上传 stage's
    // 网盘目录 — feed it into the ③ release stage even when upload itself is
    // skipped this run (the floor only depends on authorization + directory).
    stages.release.params.remoteDir = typeof stages.upload.params.remoteDir === 'string'
      ? stages.upload.params.remoteDir
      : '';
    ...
    if (stages.pack.enabled) {
      const validationMode = p.validationMode ?? stages.pack.params.validationMode ?? 'balanced';
      if (!['balanced', 'strict', 'compatibility'].includes(validationMode)) throw badRequest('validationMode 必须是 balanced / strict / compatibility');
      stages.pack.params.validationMode = validationMode;
      const cookErrorPolicy = p.cookErrorPolicy ?? stages.pack.params.cookErrorPolicy ?? (validationMode === 'compatibility' ? 'ignore' : 'surface');
      if (!['surface', 'ignore'].includes(cookErrorPolicy)) throw badRequest('cookErrorPolicy 必须是 surface 或 ignore');
      stages.pack.params.cookErrorPolicy = cookErrorPolicy;
    }
    ...
    if (stages.pack.enabled) {
      ...
      if (!engine.dir) {
        throw diagnosticError('UE 打包阶段已启用，但未能定位 UE 引擎（自动识别失败且未手动保存）。请在本页或「UE 打包」页填写并保存引擎目录。', {
          code: 'engine-not-found', severity: 'blocker', title: '总览无法启动 UE 打包',
          reason: '自动识别、项目保存路径均未找到有效 RunUAT.bat。', risk: 'UE 阶段无法执行，后续依赖打包产物的阶段也无法开始。',
          actions: [{ id: 'set-engine', label: '填写并保存引擎目录', kind: 'primary' }], canContinue: false,
        });
      }
      if (!pc.closeEditor && await editorProcessesRunning() && validationMode === 'strict') {
        throw diagnosticError('Unreal Editor 正在运行，严格模式禁止在编辑器开启时打包。', {
          code: 'editor-running', severity: 'blocker', title: 'Unreal Editor 正在运行',
          reason: '编辑器可能持有 Binaries、Intermediate 或 Cook 目录的文件锁。',
          risk: 'UAT 可能失败，或读取到未保存/不一致的工程状态。',
    ...
    if (stages.release.enabled) {
      ...
      if (!stages.pack.enabled) {
        const sourceDir = rc.sourceDir ? projectPathOf(projectRoot, rc.sourceDir) : path.join(projectRoot, 'Saved', 'Windows');
        if (sourceDir === null) throw badRequest('sourceDir 必须位于当前 UE 工程目录内。');
        const hasBuild = !!(await findPackagedExecutable(sourceDir, gameName)).executable;
        if (!hasBuild) {
          throw Object.assign(new Error(`复制压缩阶段已启用，但发布来源中未找到可执行文件：${sourceDir}。请先执行 UE 打包，或指定工程目录内的 sourceDir。`), { code: 'artifact-not-found', status: 409 });
        }
      }
    }
    if (stages.upload.enabled) {
      ...
      if (wantsAuto && stages.release.enabled && stages.release.params.zip === false) {
        throw conflictError('上传阶段需要第三阶段生成 zip；请勾选「复制后压缩」，或在上传参数中指定本地文件。');
      }
      if (wantsAuto && !stages.release.enabled) {
        const releases = await scanReleases(path.join(projectRoot, 'Saved'), await releasePrefixesForProjectAsync(projectRoot, projectName));
        if (!releases.some((r) => r.isZip === true && !r.isDir)) {
    ...
    (async () => {
      try {
        for (const id of enabledIds) {
          ...
          let stageError;
          try {
            const stageParams = { ...stages[id].params };
            if (id === 'upload' && !String(stageParams.localPath ?? '').trim()) {
              const releaseStage = job.stages.find((candidate) => candidate.id === 'release');
              const producedZip = releaseStage?.result?.zip;
              if (typeof producedZip === 'string' && producedZip.trim() !== '') {
                stageParams.localPath = producedZip;
                job.lines.push(`上传阶段使用本轮发布压缩包：${producedZip}`);
              }
            }
            const started = await OVERVIEW_STAGE_STARTERS[id]({
              ...stageParams,
              projectRoot,
              [INTERNAL_RELEASE_PROJECT_NAME]: projectName,
            });
            subJobId = started.jobId;
            job.stageJobId = subJobId;
            const sub = jobs.get(subJobId);
            if (!sub) throw new Error('阶段任务未创建');
            // upload sub-jobs resolve their destination (incl. auto-matched zip
            // name) synchronously at start — surface it on the stage snapshot
            if (sub.remotePath !== undefined) st.remotePath = sub.remotePath;
            st.policy = sub.policy;
            ...
            while (sub.running) {
              ...
              st.progress = sub.progress; // raw progress (sent/total/etaSec/speed) for the upload-stage stats line
          ...
          if (stageError) {
            ...
            st.error = stageError;
            if (subJobId) {
              const failedSub = jobs.get(subJobId);
              if (failedSub?.errorDetails) st.errorDetails = failedSub.errorDetails;
              if (failedSub?.diagnostics?.length) st.diagnostics = failedSub.diagnostics.slice(-20);
              if (failedSub?.errorDetails) job.errorDetails = failedSub.errorDetails;
              if (failedSub?.diagnostics?.length) job.diagnostics.push(...failedSub.diagnostics.slice(-20));
            }
          ...
          st.status = 'done';
          if (subJobId) {
            const finishedSub = jobs.get(subJobId);
            if (finishedSub?.diagnostics?.length) st.diagnostics = finishedSub.diagnostics.slice(-20);
            if (finishedSub?.result) st.result = finishedSub.result;
            if (finishedSub?.policy) st.policy = finishedSub.policy;
          }
          st.percent = 100;
          st.elapsedMs = Date.now() - st.startedAt;
          doneCount += 1;
  ...
  return defineTool({
    ...
    async execute(args, exec) {
      try {
        ...
        if (error instanceof Error && error.diagnostic) {
...
export function apply(ctx) {
  ...
  ctx.tools.register(tool({
    ...
    params: {
      ...
      validationMode: { type: 'string', enum: ['balanced', 'strict', 'compatibility'], description: 'Validation policy: balanced warns and continues, strict blocks safety risks, compatibility follows the UE Editor workflow more closely.' },
      cookErrorPolicy: { type: 'string', enum: ['surface', 'ignore'], description: 'Cook error policy: surface (default) keeps errors visible; ignore passes -IgnoreCookErrors and may produce an incomplete package.' },
  ...
  ctx.tools.register(tool({
    name: 'xcc_release',
    description: 'Start the project release job: copy the packaged build Saved\\Windows to a release folder named from the current project and date (first of the day has no number; later releases use -N), then zip it as the same-name .zip (release folder as the zip top entry). Returns { jobId, name, dir, zip }; poll with xcc_job.',
    params: {
      number: { type: 'integer', description: 'Optional manual release number (>=1). Empty = auto (first of day plain, then -1, -2...).' },
      sourceDir: { type: 'string', description: 'Optional project-relative source directory containing the packaged executable (default Saved\\Windows).' },
  ...
  ctx.tools.register(tool({
    name: 'xcc_job',
    description: 'Poll an xcc_pack / xcc_web_build / xcc_release job by jobId: running state, stage, recent log lines, exit code, error, elapsed time, and structured diagnostics (reason, risk, actions).',
    params: {
