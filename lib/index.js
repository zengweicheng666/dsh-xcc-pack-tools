...
/**
 * Sample a process's cumulative IO write bytes (socket + file writes) via
 * PowerShell's Process.IO counter. Used to derive upload progress/speed/ETA
 * since bdpan prints no progress in non-interactive mode. Returns null when
 * the process is gone or the counter is unavailable.
 */
function readProcessIoBytes(pid) {
  return new Promise((resolve) => {
    const child = spawn('powershell.exe', ['-NoProfile', '-Command', `(Get-Process -Id ${Number(pid)} -ErrorAction SilentlyContinue).IO.WriteBytesCount`], { windowsHide: true });
    let out = '';
