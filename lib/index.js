...
/**
 * Sample a process's cumulative IO write bytes via the CIM per-process raw
 * counter (Win32_PerfRawData_PerfProc_Process.IOWriteBytesPersec). Used to
 * derive upload progress/speed/ETA since bdpan prints no progress in
 * non-interactive mode.
 * NOTE: (Get-Process).IO.WriteBytesCount returns EMPTY in PowerShell 5.1
 * (ProcessInfo wrapper), and the formatted counter's rate reads 0 on first
 * query — the raw counter is the reliable source (verified: a 2MB file
 * write yields exactly delta=2097152 with IORead delta=0).
...
function readProcessIoBytes(pid) {
  return new Promise((resolve) => {
    const child = spawn('powershell.exe', ['-NoProfile', '-Command',
      `(Get-CimInstance Win32_PerfRawData_PerfProc_Process -Filter "IDProcess=${Number(pid)}" -ErrorAction SilentlyContinue).IOWriteBytesPersec`],
    { windowsHide: true });
    let out = '';
