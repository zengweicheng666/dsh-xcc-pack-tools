...
function releasePrefixesForProject(projectRoot) {
  const current = normalizeReleasePrefix(releaseProjectName(projectRoot));
  const legacy = normalizeReleasePrefix(RELEASE_PREFIX);
  return current.toLowerCase() === legacy.toLowerCase() ? [current] : [current, legacy];
}
...
function buildApi(ctx) {
  ...
  api.root = async (p) => {
    ...
    const gameName = uproject ? path.basename(uproject, path.extname(uproject)) : releaseProjectName(projectRoot);
    const projectName = releaseProjectName(projectRoot);
    ...
    const releases = await scanReleases(path.join(projectRoot, 'Saved'), releasePrefixesForProject(projectRoot));
  ...
  api.releases = async (p) => {
    const cwd = sessionCwdOf(ctx, p.sessionId, p.cwd);
    const projectRoot = await findProjectRoot(p.projectRoot || cwd);
    return { releases: await scanReleases(path.join(projectRoot, 'Saved'), releasePrefixesForProject(projectRoot)) };
  };
  ...
  api.run = async (p) => {
    ...
    const { uproject } = await readUprojectInfo(projectRoot);
    const gameName = uproject ? path.basename(uproject, path.extname(uproject)) : releaseProjectName(projectRoot);
    const releasePrefix = normalizeReleasePrefix(releaseProjectName(projectRoot));
    const releasePrefixes = releasePrefixesForProject(projectRoot);
    let exe;
    let dir;
    if (p.target === 'build') {
  ...
  api.uploadStart = async (p) => {
    ...
    const remoteDir = String(p.remoteDir ?? '').trim() || `${releaseProjectName(projectRoot)}/`;
