...
import { decodeLine, localDateStamp, scanReleases, computeReleaseName } from './pure.js';
  ...
  api.releaseStart = async (p) => {
    ...
    (async () => {
      try {
        ...
        job.stage = 'done';
        job.result = { dir: dst, zip: zipPath, name: info.name };
        job.lines.push(`发布完成：${zipPath}`);
        finishJob(job, { exitCode: 0 });
      } catch (e) {
        finishJob(job, { error: e });
      }
    })();
    return { jobId: job.jobId, kind: 'release', name: info.name, dir: dst, zip: doZip ? zipPath : undefined };
  };
