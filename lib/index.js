...
function releasePrefixesForProject(projectRoot, projectName = releaseProjectName(projectRoot)) {
  ...
  const candidates = [projectName, releaseProjectName(projectRoot), RELEASE_PREFIX];
  const seen = new Set();
  const prefixes = [];
...
function buildApi(ctx) {
  ...
  api.root = async (p) => {
    ...
    const releases = await scanReleases(path.join(projectRoot, 'Saved'), releasePrefixesForProject(projectRoot, projectName));
  ...
  api.releases = async (p) => {
    ...
    const projectRoot = await findProjectRoot(p.projectRoot || cwd);
    const projectName = await effectiveReleaseProjectName(projectRoot);
    return { releases: await scanReleases(path.join(projectRoot, 'Saved'), releasePrefixesForProject(projectRoot, projectName)) };
  };
  ...
  api.run = async (p) => {
    ...
    const gameName = uproject ? path.basename(uproject, path.extname(uproject)) : projectName;
    const releasePrefix = normalizeReleasePrefix(projectName);
    const releasePrefixes = releasePrefixesForProject(projectRoot, projectName);
    let exe;
    let dir;
    if (p.target === 'build') {
