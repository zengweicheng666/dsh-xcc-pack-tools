...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function PackPanel(props) {
      ...
      var [packOpts, setPackOpts] = useState({ buildConfig: "Development", skipCompile: false, skipWebBuild: false, cleanCook: false, closeEditor: false });
      ...
      useEffect(function () {
        ...
        var pollMethod = job.kind === "pack" ? "packPoll" : job.kind === "webBuild" ? "webBuildPoll" : "releasePoll";
        var tick = function () {
      ...
      // ------------------------------------------------ view: pack
      function PackView() {
        var busy = !!(job && job.kind === "pack" && job.state && job.state.running) || (job && job.kind === "pack" && !job.state);
        return h("div", {},
          h("div", { className: "dsh-pack-card" },
            ...
            h("div", { className: "dsh-pack-note" }, "完整打包内部会先构建生产版 Web UI（package.ps1 既有行为）；UAT 耗时约 1 小时级，日志实时流式显示。"),
            h("div", { className: "dsh-pack-row" },
      ...
      // ------------------------------------------------ view: web
      function WebView() {
        var busy = !!(job && job.kind === "webBuild");
        return h("div", {},
          h("div", { className: "dsh-pack-card" },
            h("h4", {}, "Web 构建（Vite）"),
            h("div", { className: "dsh-pack-row" },
      ...
      function ReleaseView() {
        var busy = !!(job && job.kind === "release");
        var collide = preview && preview.collisions && preview.collisions.length > 0;
        var releases = root ? (root.releases || []) : [];
        return h("div", {},
          h("div", { className: "dsh-pack-card" },
            h("h4", {}, "发布命名 + 压缩"),
            h("div", { className: "dsh-pack-kv" }, "来源：", h("b", { className: "dsh-pack-path", title: root ? root.outputDir : "" }, root ? root.outputDir : "—")),
            h("div", { className: "dsh-pack-row" },
            ...
            h("div", { className: "dsh-pack-note" }, "流程：复制 Saved\\Windows 全部内容 → Saved\\" + (preview ? preview.name : "XCC-Deluxe-{日期}(-N)") + "\\，再压缩为同名 .zip（zip 内含同名顶层文件夹，与历史产物结构一致）。复制 + 压缩约需 10-20 分钟（产物约 5GB）。"),
            h("div", { className: "dsh-pack-row" },
      ...
      var stageLabel = job && job.state ? (
        ...
        job.state.stage === "building" ? "Web 构建中…" : (job.state.stage || "运行中…")
      ) : "";
      ...
      return h("div", { className: "dsh-pack" },
        ...
        job && job.state && (job.state.lines || []).length > 0
          ? h("div", { className: "dsh-pack-loghead" },
              h("span", { className: "dsh-pack-spin" }), stageLabel, " · " + fmtTime(job.state.elapsedMs),
              h("button", { className: "dsh-pack-actbtn", style: { marginLeft: "auto" }, disabled: !job.state.running, onClick: cancelJob }, "取消"))
          : null,
        job && job.state && (job.state.lines || []).length > 0
