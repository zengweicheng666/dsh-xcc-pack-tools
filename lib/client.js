  ...
  factory: (require) => {
    ...
    function PackPanel(props) {
      ...
      var [ovOpts, setOvOpts] = useState({
        ...
        web: { enabled: true, mode: "dev", targetDir: "" },
      ...
      function ovStageState(id) {
        ...
        if (ovJob && ovJob.state && Array.isArray(ovJob.state.stages)) {
          ...
          if (s && (live || s.status === "done" || s.status === "failed")) {
            return { status: s.status, pct: typeof s.percent === "number" ? s.percent : null, label: s.label, err: s.error, elapsedMs: s.elapsedMs };
          }
        }
        var ent = jobs[OV_STAGE_KIND[id]];
        if (ent && ent.state) {
          var st = ent.state;
          if (st.running) return { status: "running", pct: (st.progress && typeof st.progress.percent === "number") ? st.progress.percent : null, label: st.progress && st.progress.label };
          if (st.killed) return { status: "failed", pct: null, err: "已取消" };
          if (st.error) return { status: "failed", pct: null, err: st.error };
          if (st.done) return { status: "done", pct: 100 };
        }
            ...
            params = [
              h("div", { className: "dsh-pack-row" },
                ...
                h("select", { className: "dsh-pack-select", disabled: anyRunning, value: o.mode,
                  ...
                  h("option", { value: "dev" }, "开发版 dev（默认，仓库规则）"),
                  h("option", { value: "prod" }, "生产版 prod（仅生产发布）"))),
              h("div", { className: "dsh-pack-row" },
              ...
              h("div", { className: "dsh-pack-kv" }, "版本：", h("b", {}, root && root.webVersion ? root.webVersion.current + " → 构建后 " + root.webVersion.next : "—")),
            ];
          } else if (id === "release") {
            params = [
              h("div", { className: "dsh-pack-row" },
