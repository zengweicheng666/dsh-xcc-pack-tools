...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      ".dsh-pack-prog{margin-top:6px;background:#0d1117;border:1px solid rgba(148,163,184,.2);border-radius:6px;padding:6px 8px}",
    ...
    function PackPanel(props) {
      ...
      function startJob(kind, body, doneName) {
        ...
        var method = kind === "pack" ? "packStart"
          ...
          : kind === "bdpanInstall" ? "bdpanInstallStart"
          : "releaseStart";
      ...
      function UploadView() {
        ...
        return h("div", {},
          ...
          h("div", { className: "dsh-pack-card" },
            ...
            h("div", { className: "dsh-pack-note" }, "上传直接使用上面的压缩包文件（不复制、不压缩），后台任务日志实时流式显示（大文件可能耗时较长）；单文件上传目标必须是完整文件路径，插件自动拼上文件名。"),
            h("div", { className: "dsh-pack-row" },
      ...
      var kindLabel = job ? (
        ...
        job.kind === "release" ? "发布" :
        job.kind === "upload" ? "上传" :
        job.kind === "bdpanInstall" ? "安装 CLI" : job.kind
      ) : "";
      ...
      return h("div", { className: "dsh-pack" },
        ...
        job && job.state && (job.state.lines || []).length > 0
          ? h("div", { className: "dsh-pack-loghead" },
              ...
              job.state.running
                ? kindLabel + " · " + stageLabel + " · " + fmtTime(job.state.elapsedMs)
                : jobDoneLabel + "（" + kindLabel + "）· 耗时 " + fmtTime(job.state.elapsedMs),
              job.state.running
                ? h("button", { className: "dsh-pack-actbtn", style: { marginLeft: "auto" }, onClick: cancelJob }, "取消")
                : null)
          : null,
        job && job.state && job.kind === "upload" && job.state.progress && job.state.progress.total
          ? (function () {
              var p = job.state.progress;
              var pct = p.percent === undefined ? 0 : p.percent;
              var parts = [];
              if (p.percent !== undefined) parts.push(p.percent + "%");
              if (p.speed) parts.push(fmtSpeed(p.speed));
              if (p.sent !== undefined && p.total) parts.push("已传 " + fmtSize(p.sent) + " / " + fmtSize(p.total));
              if (p.sent !== undefined && p.total && p.sent < p.total) parts.push("剩余 " + fmtSize(p.total - p.sent));
              if (p.etaSec !== undefined && p.etaSec > 0) parts.push("约 " + fmtTime(p.etaSec * 1000));
              if (p.etaSec === 0) parts.push("即将完成");
              return h("div", { className: "dsh-pack-prog" },
                h("div", { className: "dsh-pack-progbar" }, h("i", { style: { width: pct + "%" } })),
