  ...
  factory: (require) => {
    ...
    function PackPanel(props) {
          ...
          post(pollMethod, { jobId: job.jobId })
            .then(function (st) {
              ...
              setJobs(function (all) { var next = Object.assign({}, all); next[job.kind] = { jobId: job.jobId, state: st }; return next; });
                 ...
                 var previous = next[job.kind] || [];
                 var incoming = st.lines || [];
                 next[job.kind] = previous.concat(incoming.filter(function (line) { return previous.indexOf(line) < 0; }));
                 return next;
               });
      ...
      function renderTaskLogs() {
        var kinds = [{ id: "pack", name: "UE 打包" }, { id: "webBuild", name: "Web 构建" }, { id: "release", name: "发布" }, { id: "upload", name: "上传" }];
        return kinds.map(function (item) {
          var entry = jobs[item.id];
          var lines = (entry && entry.state && entry.state.lines) || taskLogs[item.id] || [];
          if (!entry && lines.length === 0) return null;
          var p = entry && entry.state && entry.state.progress;
          return h("div", { key: item.id, className: "dsh-pack-task" },
            h("div", { className: "dsh-pack-loghead" }, item.name, h("button", { className: "dsh-pack-mini", style: { marginLeft: "auto" }, onClick: function () { clearLog(item.id); } }, "清空日志")),
            p ? h("div", { className: "dsh-pack-prog" }, h("div", { className: "dsh-pack-progbar" }, h("i", { style: { width: ((p.percent == null ? 0 : p.percent) + "%") } })), h("div", { className: "dsh-pack-progtext" }, p.label || "处理中…")) : null,
            h("div", { className: "dsh-pack-resizer", onPointerDown: function (e) { resizeTaskLog(item.id, e); }, title: "上下拖动调整日志高度" }),
             h("div", { className: "dsh-pack-log", style: { height: (taskLogHeights[item.id] || 180) + "px" } }, lines.join("\n")));
        });
      }
                ...
                releases.map(function (r) {
                  return h("div", { className: "dsh-pack-relrow", key: r.path, title: r.path },
                    ...
                    r.isDir
                      ...
                      : h("button", { className: "dsh-pack-actbtn", style: { marginLeft: "auto" }, onClick: function () { clearLog(job.kind); } }, "清空日志"));
                }))));
      }
      ...
      return h("div", { className: "dsh-pack" },
        ...
        h("div", { className: "dsh-pack-body" },
          ...
          body),
                 h("div", { className: "dsh-pack-separator" }),
         h("div", { className: "dsh-pack-tasklogs" }, renderTaskLogs()),
