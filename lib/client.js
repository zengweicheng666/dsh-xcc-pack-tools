      ...
      function ovStageState(id) {
        ...
        if (ovJob && ovJob.state && Array.isArray(ovJob.state.stages)) {
          ...
          if (s && (live || s.status === "done" || s.status === "failed")) {
            return { status: s.status, pct: typeof s.percent === "number" ? s.percent : null, label: s.label, err: s.error, elapsedMs: s.elapsedMs, remotePath: s.remotePath, progress: s.progress };
          }
        }
        var ent = jobs[OV_STAGE_KIND[id]];
        if (ent && ent.state) {
          var st = ent.state;
          if (st.running) return { status: "running", pct: (st.progress && typeof st.progress.percent === "number") ? st.progress.percent : null, label: st.progress && st.progress.label, remotePath: st.remotePath, progress: st.progress };
          if (st.killed) return { status: "failed", pct: null, err: "已取消", remotePath: st.remotePath, progress: st.progress };
          if (st.error) return { status: "failed", pct: null, err: st.error, remotePath: st.remotePath, progress: st.progress };
          if (st.done) return { status: "done", pct: 100, remotePath: st.remotePath, progress: st.progress };
        }
