...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      ".dsh-pack-mini.danger{color:#ffa198;border-color:rgba(248,81,73,.4)}",
      ".dsh-pack-mini.danger:hover{color:#ff7b72;border-color:rgba(248,81,73,.7);background:rgba(248,81,73,.1)}",
    ...
    function PackPanel(props) {
      ...
      var [deleteBusy, setDeleteBusy] = useState(""); // name of the release being deleted ("" = idle)
      ...
      // newest <project>-<yyyyMMdd>(-N).zip already on the netdisk, per source
      // view: { name, error } | null (null = unauthorized / not yet queried)
      var [uploadRemote, setUploadRemote] = useState(null);
      var [ovRemote, setOvRemote] = useState(null);
      ...
      var uploadDirRef = useRef(""); // live mirror of the upload tab's remoteDir (for reload's nextName)
      uploadDirRef.current = remoteDir;
      ...
      var reload = useCallback(function () {
        post("root", base)
          .then(function (v) {
            ...
            var date = v.now;
            return post("nextName", Object.assign({}, base, { date: date, remoteDir: uploadDirRef.current })).then(function (nv) {
              setPreview(nv);
              setPreviewErr("");
              return releases;
            }).catch(function (e) {
      ...
      // server remains authoritative for auto-numbering, collision checks,
      // and the netdisk floor (网盘目录 + 授权状态 feed in as dependencies).
      ...
      useEffect(function () {
        ...
        var timer = setTimeout(function () {
          post("nextName", Object.assign({}, base, { date: root.now, number: number, remoteDir: remoteDir }))
      ...
      }, [root && root.projectRoot, root && root.projectName, root && root.now, releaseSignature, manualNumber, remoteDir, baidu && baidu.authorized]);
      ...
      useEffect(function () {
        ...
        var cancelled = false;
        var timer = setTimeout(function () {
          post("nextName", Object.assign({}, base, { date: root.now, number: number, remoteDir: ovOpts.upload.remoteDir || "" }))
        ...
        return function () { cancelled = true; clearTimeout(timer); };
      }, [root && root.projectRoot, root && root.projectName, root && root.now, releaseSignature, ovOpts.release.number, ovOpts.upload.remoteDir, baidu && baidu.authorized]);

      // resume a server-side running job after a page refresh
      useEffect(function () { resumeActiveJob(); }, []);

      // refresh Baidu open-platform status AND the netdisk-latest name
      // whenever the upload/overview views open
      useEffect(function () {
        if (view === "upload" || view === "overview") {
          loadBaiduStatus();
          if (view === "upload") loadUploadRemote();
          else loadOvRemote();
        }
      }, [view]);

      // re-query the netdisk-latest name when either 网盘目录 input changes
      // (debounced; unauthorized configs short-circuit server-side)
      useEffect(function () {
        if (!root) return;
        var t = setTimeout(function () { loadUploadRemote(); }, 300);
        return function () { clearTimeout(t); };
      }, [remoteDir, root && root.projectRoot, root && root.projectName]);

      useEffect(function () {
        if (!root) return;
        var t = setTimeout(function () { loadOvRemote(); }, 300);
        return function () { clearTimeout(t); };
      }, [ovOpts.upload.remoteDir, root && root.projectRoot, root && root.projectName]);
          ...
          post(pollMethod, { jobId: job.jobId })
            .then(function (st) {
              ...
              if (!st.running && st.done) {
                ...
                reload();
                // the finished run may have produced/uploaded a new zip — the
                // netdisk-latest line and release preview floor must catch up
                loadUploadRemote();
                loadOvRemote();
              }
            })
      ...
      // Delete one existing release: the Saved\<name> folder and its
      // same-name .zip are removed together (whichever exists). The server
      // re-validates the name against the project's release prefixes and
      // refuses while a release/overview/upload job is running.
      function removeRelease(r) {
        var name = r.name;
        if (!window.confirm("确定删除发布「" + name + "」？将一并删除 Saved\\" + name + " 文件夹与同名 .zip（若存在），此操作不可恢复。")) return;
        setErr(""); setNotice(""); setDeleteBusy(name);
        post("releaseDelete", Object.assign({}, base, { name: name }))
          .then(function (v) {
            var parts = [v.deleted.dir ? "文件夹" : null, v.deleted.zip ? "zip" : null].filter(Boolean);
            setNotice("已删除发布「" + name + "」（" + (parts.join(" + ") || "无产物") + "）");
            return reload();
          })
          .catch(function (e) { setErr(e.message); })
          .finally(function () { setDeleteBusy(""); });
      }
      ...
      function ovStagePayload(id) {
        ...
        // the release-name netdisk floor compares against the ④ upload
        // stage's 网盘目录 (even when upload is skipped this run)
        if (id === "release") o.remoteDir = String(ovOpts.upload.remoteDir || "");
        return o;
      }
        ...
        function stageCard(id) {
          ...
          if (id === "pack") {
            ...
            params = [
              ...
              releasePreview && releasePreview.netdisk && releasePreview.netdisk.adjusted
                ? h("div", { className: "dsh-pack-note" }, "已按网盘最新「" + releasePreview.netdisk.latest + "」自动 +1（避免与网盘重名）")
                : null,
            ];
          } else {
            params = [
              ...
              baidu && baidu.authorized && ovRemote
                ? h("div", { className: "dsh-pack-kv" }, "网盘最新：",
                    ovRemote.error
                      ? h("span", { className: "dsh-pack-note", style: { margin: 0 } }, "获取失败：" + ovRemote.error)
                      : h("b", { className: "dsh-pack-uploadto", title: "按文件名（项目+日期+编号）取最新，忽略上传/修改时间" }, ovRemote.name || "—"))
                : null,
              upTarget
            ...
            h("div", { className: "dsh-ov-body" },
              ...
              s.status === "running" && id === "upload" && s.progress && typeof s.progress.sent === "number" && typeof s.progress.total === "number"
                ? h("div", { className: "dsh-pack-kv", style: { marginTop: 6, justifyContent: "center" } }, "已上传：", h("b", {}, fmtSize(s.progress.sent)), " / ", h("b", {}, fmtSize(s.progress.total)),
                    typeof s.progress.speed === "number" && s.progress.speed > 0 ? h("span", {}, " · " + fmtSpeed(s.progress.speed)) : null,
      ...
      function ReleaseView() {
        ...
        return h("div", {},
          h("div", { className: "dsh-pack-card" },
            ...
            root && root.projectNameSource
              ? h("div", { className: "dsh-pack-note", style: { marginTop: 0 } },
                  root.projectNameSource === "custom" ? "当前使用自定义项目名。"
                    : root.projectNameSource === "history" ? "自动沿用最近用过的名字（可在上方修改并保存）。"
                    : "自动识别（工程目录名）。")
              : null,
            h("div", { className: "dsh-pack-row" },
            ...
            preview && preview.netdisk && preview.netdisk.adjusted
              ? h("div", { className: "dsh-pack-note" }, "已按网盘最新「" + preview.netdisk.latest + "」自动 +1（避免与网盘重名）")
              : null,
            ...
            h("div", { className: "dsh-pack-row" },
              h("button", { className: "dsh-pack-actbtn primary", disabled: !root || busy || collide || !!previewErr || (root && !root.hasBuild), onClick: function () {
                startJob("release", { number: manualNumber.trim() === "" ? undefined : manualNumber.trim(), zip: true, remoteDir: remoteDir });
              } }, busy ? "发布中…" : "复制并压缩"),
              h("button", { className: "dsh-pack-actbtn", disabled: !root || busy || collide || !!previewErr || (root && !root.hasBuild), onClick: function () {
                startJob("release", { number: manualNumber.trim() === "" ? undefined : manualNumber.trim(), zip: false, remoteDir: remoteDir });
              } }, busy ? "发布中…" : "仅复制命名"))),
          h("div", { className: "dsh-pack-card" },
            h("h4", {}, "现有发布（Saved\\）"),
            releases.length === 0
                ...
                releases.map(function (r) {
                  return h("div", { className: "dsh-pack-relrow", key: r.path, title: r.path },
                    ...
                    r.isDir
                      ...
                      : null,
                    h("button", { className: "dsh-pack-mini danger", title: "删除该发布（Saved\\" + r.name + " 文件夹 + 同名 .zip）", disabled: busy || anyJobBusy || deleteBusy !== "",
                      onClick: function () { removeRelease(r); } }, deleteBusy === r.name ? "删除中…" : "删除"));
                }))),
      ...
      // newest <project>-<yyyyMMdd>(-N).zip already in the 网盘目录 — one
      // loader per source view so both the 上传 tab and the 总览 upload card
      // show the value for THEIR directory input
      function loadUploadRemote() {
        post("baiduDirLatest", Object.assign({}, base, { remoteDir: remoteDir }))
          .then(function (v) {
            if (!v.authorized) { setUploadRemote(null); return; }
            setUploadRemote({ name: v.name || null, error: v.error || null });
          })
          .catch(function (e) { setUploadRemote({ name: null, error: e.message }); });
      }

      function loadOvRemote() {
        post("baiduDirLatest", Object.assign({}, base, { remoteDir: ovOpts.upload.remoteDir || "" }))
          .then(function (v) {
            if (!v.authorized) { setOvRemote(null); return; }
            setOvRemote({ name: v.name || null, error: v.error || null });
          })
          .catch(function (e) { setOvRemote({ name: null, error: e.message }); });
      }
      ...
      function submitAuthCode() {
        ...
        post("baiduAuthorizeCode", { code: authCode.trim() })
          .then(function () { setAuthCode(""); setAuthUrl(""); setNotice("百度开放平台授权成功 ✔"); loadBaiduStatus(); loadUploadRemote(); loadOvRemote(); })
          .catch(function (e) { setErr(e.message); })
          .finally(function () { setAuthBusy(false); });
      }
      ...
      function UploadView() {
        ...
        return h("div", {},
          ...
          h("div", { className: "dsh-pack-card" },
            ...
            authorized && uploadRemote
              ? h("div", { className: "dsh-pack-kv" }, "网盘最新：",
                  uploadRemote.error
                    ? h("span", { className: "dsh-pack-note", style: { margin: 0 } }, "获取失败：" + uploadRemote.error)
                    : h("b", { className: "dsh-pack-uploadto", title: "按文件名（项目+日期+编号）取最新，忽略上传/修改时间" }, uploadRemote.name || "—"))
              : null,
