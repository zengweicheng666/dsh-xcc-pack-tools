...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Forecast the zip name the release stage will produce for root.now
    // (mirror of lib/pure.js computeReleaseName auto/manual rules): plain
    // XCC-Deluxe-{date} first of the day, then -1, -2, … (plain counts as 0).
    // Shown as the 总览 upload destination until a real upload sub-job reports
    // its authoritative remotePath.
    function ovForecastReleaseName(root, manualNumberRaw) {
      if (!root || !root.now) return "";
      var sameDay = Array.isArray(root.releases)
        ? root.releases.filter(function (r) { return r.date === root.now; })
        : [];
      var numText = String(manualNumberRaw || "").trim();
      var number = null;
      if (numText !== "" && Number.isInteger(Number(numText)) && Number(numText) >= 1) {
        number = Number(numText);
      } else if (sameDay.length > 0) {
        var maxN = 0;
        sameDay.forEach(function (r) { maxN = Math.max(maxN, r.number || 0); });
        number = maxN + 1;
      }
      return "XCC-Deluxe-" + root.now + (number === null ? "" : "-" + number) + ".zip";
    }
    ...
    function PackPanel(props) {
      ...
      var [ovOpts, setOvOpts] = useState({
        pack: { enabled: true, buildConfig: "Development", skipCompile: false, cleanCook: false, closeEditor: false },
        web: { enabled: true, mode: "prod", targetDir: "" },
        release: { enabled: true, number: "", zip: true, zipTool: "" },
        upload: { enabled: true, localPath: "", remoteDir: "XCC-Deluxe/" },
      });
      ...
      var OV_STAGE_SUB = {
        ...
        release: "XCC-Deluxe-{yyyyMMdd}(-N) + zip",
        upload: "直连 PCS · 分片续传",
      };
      ...
      function ovStageState(id) {
        ...
        if (ovJob && ovJob.state && Array.isArray(ovJob.state.stages)) {
          ...
          if (s && (live || s.status === "done" || s.status === "failed")) {
            return { status: s.status, pct: typeof s.percent === "number" ? s.percent : null, label: s.label, err: s.error, elapsedMs: s.elapsedMs, remotePath: s.remotePath };
          }
        }
        var ent = jobs[OV_STAGE_KIND[id]];
        if (ent && ent.state) {
          var st = ent.state;
          if (st.running) return { status: "running", pct: (st.progress && typeof st.progress.percent === "number") ? st.progress.percent : null, label: st.progress && st.progress.label, remotePath: st.remotePath };
          if (st.killed) return { status: "failed", pct: null, err: "已取消", remotePath: st.remotePath };
          if (st.error) return { status: "failed", pct: null, err: st.error, remotePath: st.remotePath };
          if (st.done) return { status: "done", pct: 100, remotePath: st.remotePath };
        }
      ...
      function startOverview() {
        ...
        post("overviewStart", Object.assign({}, base, { stages: stages }))
          ...
          .catch(function (e) { setErr(e.message); if (e.code === "busy") resumeActiveJob(); });
      }
      ...
      function OverviewView() {
        ...
        function stageCard(id) {
          ...
          // zip name included); before that, forecast the file the pipeline will
          // upload — explicit localPath, the release stage's next name when this
          // pipeline produces a fresh zip, else the existing latest zip.
          var upTarget = null;
          if (id === "upload") {
            if (s.remotePath) {
              ...
              } else if (ovOpts.release.enabled && root) {
                // release stage runs earlier in this pipeline → forecast its name
                upFile = ovForecastReleaseName(root, ovOpts.release.number);
              ...
              var upDir = String(o.remoteDir || "").trim() || "XCC-Deluxe/";
          ...
          if (id === "pack") {
            ...
            params = [
              ...
              h("div", { className: "dsh-pack-row" },
                h("button", { className: "dsh-pack-mini ok", title: "启动 Saved\\Windows\\XCC.exe", disabled: anyRunning || !root || !root.hasBuild || runBusy,
                  onClick: function () { runGame("build"); } }, "运行 XCC.exe")),
            ];
          } else {
            params = [
              h("div", { className: "dsh-pack-row" },
              ...
              h("div", { className: "dsh-pack-row" },
                ...
                h("input", { className: "dsh-pack-input", disabled: anyRunning, placeholder: "相对应用根，如 XCC-Deluxe/", value: o.remoteDir,
                  onChange: function (e) { setOvStage("upload", { remoteDir: e.target.value }); } })),
              upTarget
                ? h("div", { className: "dsh-pack-kv" }, "上传到：", h("b", { className: "dsh-pack-path", title: upTarget }, upTarget))
                : null,
              h("div", { className: "dsh-pack-row" },
          ...
          return h("div", { className: "dsh-ov-card" + stCls, key: id },
            ...
            h("div", { className: "dsh-ov-body" },
              h("div", { className: "dsh-ov-head" },
                ...
                h("span", { className: "dsh-ov-name" }, OV_STAGE_TITLE[id], h("small", {}, OV_STAGE_SUB[id])),
                h("span", { style: { flex: 1 } }),
              ...
              s.status === "running"
                ...
                : null,
              params,
              s.status === "failed" && s.err ? h("div", { className: "dsh-pack-err" }, s.err) : null,
              h("div", { className: "dsh-ov-foot" },
                ...
                h("span", { style: { flex: 1 } }),
                h("button", { className: "dsh-pack-mini", disabled: locked || !root, title: "仅单独运行本阶段（不串联其他阶段）",
        ...
        return h("div", {},
          h("div", { className: "dsh-ov-top" },
            ...
            h("div", { className: "dsh-pack-row", style: { marginTop: 8 } },
              h("button", { className: "dsh-pack-actbtn primary", disabled: anyRunning || !root || enabledTotal === 0, onClick: startOverview },
          ...
          root && !root.hasBuild && ovOpts.release.enabled && !ovOpts.pack.enabled
            ? h("div", { className: "dsh-pack-warn" }, "复制压缩阶段依赖已有打包产物（Saved\\Windows\\XCC.exe 不存在）。可勾选 ① UE 打包 一起执行，或先单独打包。")
            : null,
