        ...
        job && job.state && (job.state.lines || []).length > 0
          ? h("div", { className: "dsh-pack-loghead" },
              ...
              job.state.running
                ...
                : null)
          : null,
        job && job.state && job.state.progress
          ? (function () {
              var p = job.state.progress;
              var pct = (p.percent === undefined || p.percent === null) ? null : p.percent;
              var parts = [];
              if (p.label) parts.push(p.label);
              if (pct !== null) parts.push(pct + "%");
              if (p.speed) parts.push(fmtSpeed(p.speed));
              if (p.sent !== undefined && p.total) {
                var doneWord = job.kind === "upload" ? "已传" : "已完成";
              ...
              if (parts.length === 0) parts.push("处理中…");
