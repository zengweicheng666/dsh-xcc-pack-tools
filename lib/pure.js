 ...
 * Throws badRequest for traversal (`\`, `~`, `..`, leading `/`) or empty input.
 */
export function resolveRemotePath(remoteDir, zipName) {
  ...
  if (/[\\~]/.test(dir) || dir.startsWith('/') || dir.split('/').includes('..')) {
    throw Object.assign(new Error('网盘目标路径不合法：不能包含 \\、~、.. 或以 / 开头'), { code: 'bad-request', status: 400 });
  }
  const trimmed = dir.replace(/\/+$/, '');
  const remote = trimmed.toLowerCase().endsWith('.zip') ? trimmed : `${trimmed}/${zipName}`;
  if (remote.split('/').includes('..') || /[\\~]/.test(remote)) {
