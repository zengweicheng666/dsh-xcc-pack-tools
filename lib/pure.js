...
/**
 * Resolve the effective release project name from the configured inputs.
 * Priority (user-confirmed): explicit custom name → the most recent name the
 * project actually used (history tail) → the project directory name. Inputs
 * must already be validated by the caller; this function only picks.
 * Returns { name, source } with source ∈ custom | history | directory.
 */
export function resolveEffectiveReleaseProjectName({ override, history, directoryName } = {}) {
  if (typeof override === 'string' && override.trim() !== '') {
    return { name: override.trim(), source: 'custom' };
  }
  const names = (Array.isArray(history) ? history : [])
    .filter((value) => typeof value === 'string' && value.trim() !== '');
  if (names.length > 0) return { name: names[names.length - 1].trim(), source: 'history' };
  return { name: typeof directoryName === 'string' ? directoryName.trim() : '', source: 'directory' };
}

/**
 * Normalized, deduplicated release-prefix candidates for release
 * recognition: the active project name, the project directory name, and
 * persisted historical names. No built-in/default prefix is injected —
 * recognition is driven exclusively by the project's own names
 * (case-insensitive dedup, first spelling wins).
 */
export function releasePrefixCandidates({ projectName, directoryName, historicalNames } = {}) {
  const values = [projectName, directoryName, ...(Array.isArray(historicalNames) ? historicalNames : [])]
    .filter((value) => typeof value === 'string' && value.trim() !== '');
  const seen = new Set();
  const out = [];
  for (const value of values) {
    const prefix = normalizeReleasePrefix(value);
